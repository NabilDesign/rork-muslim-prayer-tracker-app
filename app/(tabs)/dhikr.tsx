...
// (Optioneel) Haptics met Expo; zonder Expo werkt alles gewoon.
...
export default function DhikrScreen() {
  ...
  // ephemeral preset routine id (wordt later opgeruimd)
  ...
  // start preset as ephemeral (niet zichtbaar in lijst)
  const startPreset = (preset: RoutineInput) => {
    const dhikrList = materializeRoutine(preset);
    // voeg meta toe om 'm te herkennen als preset
    const id: string = createRoutine({
      name: preset.name,
      dhikrList,
      // @ts-ignore
      meta: { preset: true, presetName: preset.name },
    });
    startRoutine(id);
  ...
  const setTarget = (id: string, n: number) =>
    setSelectedItems(prev =>
      prev.map(it => (it.id === id ? { ...it, target: Math.max(1, n) } : it))
    );
  ...
  const saveRoutine = () => {
    ...
    if (selectedItems.length === 0)
      return Alert.alert('Error', 'Please add at least one dhikr to the routine');

    const dhikrList = materializeRoutine({
      name: routineName.trim(),
      items: selectedItems,
    });
    const id: string = createRoutine({ name: routineName.trim(), dhikrList });
    ...
    <>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.sectionDescription}>
              Choose a preset routine to begin
            </Text>
          </View>
          ...
          <ScrollView
            ...
            contentContainerStyle={styles.routinesScrollContainer}
            ...
            {presetRoutines.map((p, idx) => (
              ...
              >
                ...
                <Text style={styles.routineDescription}>
                  {p.items.length} dhikr •{' '}
                  {p.items.reduce((s, it) => s + it.target, 0)} total
                </Text>
              </TouchableOpacity>
            ))}
          </ScrollView>

          {/* Custom routines (filter presets weg) */}
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Your Custom Routines</Text>
            <Text style={styles.sectionDescription}>
              Personalized dhikr collections
            </Text>
          </View>

          {routines.filter((r: any) => !r?.meta?.preset).length === 0 ? (
              ...
              {routines
                ...
                .map((r: any) => (
                  <View key={r.id} style={styles.customRoutineCard}>
                    <TouchableOpacity
                      style={styles.customRoutineMain}
                      onPress={() => {
                        startRoutine(r.id);
                        setShowCounter(true);
                      }}
                    ...
                    >
                      <View style={styles.customRoutineInfo}>
                        ...
                        <Text style={styles.customRoutineDescription}>
                          ...
                          {r.dhikrList.reduce(
                            (s: number, d: DhikrItem) => s + d.count,
                            0
                          )}{' '}
                          total
                        </Text>
          ...
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.sectionDescription}>
              Browse individual remembrances
            </Text>
          </View>
          ...
          <View style={styles.dhikrList}>
            {AVAILABLE_DHIKR.map((item, index) => (
              <View
                ...
                style={[
                  styles.dhikrCard,
                  index === AVAILABLE_DHIKR.length - 1 && styles.lastDhikrCard,
                ]}
              >
      ...
      >
        <View style={styles.counterContainer}>
          <TouchableOpacity
            style={styles.counterCloseButton}
          ...
          {isDone ? (
            ...
            <View style={styles.doneWrap}>
              ...
              <Text style={styles.doneSubtitle}>
                Masha’Allah — je hebt alles afgerond.
              </Text>

              <View style={styles.doneButtons}>
                <TouchableOpacity
                  style={[styles.doneBtn, styles.donePrimary]}
                  onPress={closeCounter}
                >
                  <Text style={styles.donePrimaryText}>Close</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[styles.doneBtn, styles.doneGhost]}
                  onPress={() => {
                    setIsDone(false);
                    resetRoutine();
                  }}
              ...
              <View style={styles.counterContent}>
                ...
                <View style={styles.counterControlsRow}>
                  <TouchableOpacity
                    style={[
                      styles.smallCtrlBtn,
                      { backgroundColor: '#ffffff22', borderColor: '#ffffff40' },
                    ]}
                    onPress={() => {
                      resetRoutine();
                    }}
                  ...
                  <TouchableOpacity
                    style={[
                      styles.smallCtrlBtn,
                      {
                        backgroundColor: isActive ? '#F59E0B' : '#16A34A',
                        borderColor: '#ffffff40',
                      },
                    ]}
                    onPress={() =>
                      isActive ? pauseRoutine() : startRoutine(activeRoutine.id)
                    }
                  >
                    {isActive ? (
                      <Pause color={colors.surface} size={18} />
                    ) : (
      ...
      {/* Create Routine Modal (SectionList is parent; geen nested ScrollView) */}
      <Modal
          ...
          <SectionList
              ...
              <View>
                      ...
                      return (
                        <Pressable
                          ...
                          style={({ pressed }) => [
                            styles.chip,
                            {
                              backgroundColor: selected ? '#DCFCE7' : '#F1F5F9',
                ...
                <View style={{ marginTop: 12, marginBottom: 8 }}>
                  <Text style={styles.inputLabel}>
                    Selected Dhikr ({selectedItems.length})
                  </Text>
                  {selectedItems.length > 0 ? (
                        ...
                        return (
                          <View key={it.id} style={styles.selectedItemCard}>
                            <View style={{ flex: 1 }}>
                              ...
                              <Text style={styles.selectedItemTranslit}>
                                {d.transliteration}
                              </Text>
                            </View>
                            <View style={styles.selectedItemControls}>
                              <View style={styles.targetControls}>
                                <TouchableOpacity
                                  ...
                                  onPress={() => setTarget(it.id, it.target - 1)}
                                >
                    ...
                    <Text style={styles.noSelectedItems}>
                      No dhikr selected yet
                    </Text>
                  )}
                </View>
            ...
            renderItem={({ item }) => {
              ...
              return (
                <TouchableOpacity
                  style={[
                    styles.availableDhikrCard,
                    isSel && styles.availableDhikrCardSelected,
                  ]}
                ...
                >
                  <View style={styles.availableDhikrInfo}>
                    ...
                    <Text style={styles.availableDhikrTranslit}>
                      {item.transliteration}
                    </Text>
                    <Text style={styles.availableDhikrTranslation}>
                      {item.translation}
                    </Text>
                  </View>
                  <View
                    style={[
                      styles.availableDhikrCheck,
                      isSel && styles.availableDhikrCheckSelected,
                    ]}
                  >
...
const styles = StyleSheet.create({
  ...
  removeItemButton: {
    ...
    height: 32,
    borderRadius: 16,
    backgroundColor: colors.status.missed + '15',
    alignItems: 'center',
    justifyContent: 'center',
  },
