...
export default function DhikrScreen() {
  ...
  // Current pointers
  const routineLen = activeRoutine?.dhikrList?.length ?? 0;
  const currentDhikr = activeRoutine?.dhikrList?.[currentDhikrIndex];
  const currentTarget = currentDhikr?.count ?? 0;
  const isLastItem = !!activeRoutine && currentDhikrIndex === routineLen - 1;

  // Progress anim
  const progressAnim = useRef(new Animated.Value(0)).current;
  const routineProgress = routineLen > 0 ? currentDhikrIndex / routineLen : 0;
  useEffect(() => {
...
const onSkipOrFinish = () => {
  ...
  const addItem = (id: string) => {
    ...
    const base = AVAILABLE.find(d => d.id === id);
    setSelectedItems(prev => [...prev, { id, target: base?.defaultTarget ?? 33 }]);
  };
  ...
  const saveRoutine = () => {
    ...
    setSelectedItems([]);
    if (!id) return;
    setEphemeralRoutineId(null); // custom routine, niet ephemeral
    setIsDone(false);
    setShowCounter(true);
  };

  // Routines tonen (verberg ephemeral)
                  ...
                  <TouchableOpacity style={styles.customRoutineMain} onPress={() => { startRoutine(r.id); setIsDone(false); setShowCounter(true); }} activeOpacity={0.85}>
                    <View style={styles.customRoutineInfo}>
                      ...
                      <Text style={styles.customRoutineDescription}>
                        {r.dhikrList.length} dhikr • {r.dhikrList.reduce((s: number, d: StoreDhikrItem) => s + d.count, 0)} total
                      </Text>
                    </View>
  ...
  <Text style={styles.createRoutineText}>Create Custom Routine</Text>
          ...
          <View style={styles.dhikrList}>
            {AVAILABLE.map((item, index) => (
              <View key={item.id} style={[styles.dhikrCard, index === AVAILABLE.length - 1 && styles.lastDhikrCard]}>
                ...
                <View style={styles.dhikrContentContainer}>
                  ...
                  {!!item.defaultTarget && <Text style={styles.targetText}>Recommended: {item.defaultTarget}x</Text>}
                </View>
              </View>
            ))}
          </View>
            ...
            <View style={styles.counterContent}>
              ...
              <View style={styles.progressContainer}>
                ...
                <Text style={styles.progressText}>
                  {Math.min(currentDhikrIndex + 1, routineLen)} / {routineLen} • {currentCount} / {currentTarget}
                </Text>
              </View>
              ...
              <View style={styles.counterControlsRow}>
                ...
                <TouchableOpacity style={[styles.smallCtrlBtn, { backgroundColor: '#ffffff22', borderColor: '#ffffff40' }]} onPress={onSkipOrFinish}>
                  ...
                  {selectedItems.length > 0 ? (
                    <View style={{ gap: 12 }}>
                      {selectedItems.map((it) => {
                        const d = AVAILABLE.find(x => x.id === it.id);
                        if (!d) return null;
                        return (
